...
async function init(){
  const [geo, meta, alias, flatTextures, isoGeoms] = await Promise.all([
    ...
    loadIsoGeomsFiltered(ICONS_ISO_ABS),
  ]);
  ...
  for (const f of GEO.features){
    ...
    const baseOrder = 1000 + ORDER * 3;  // minden járás külön "sáv"
    ORDER++;                              // CSAK EGYSZER növeljük node-onként
    ...
    mesh3D.renderOrder = baseOrder + 2;  // 2D után
    ...
    // kis clamp, hogy nagyon pici járásoknál se legyen "tüske"
    const zScale = Math.min(Math.max(s*rz, 0.1), 3.0);

    mesh3D.scale.set(s, s, zScale);
...
function isVeryLight(fill){
  ...
  return !!col && col.r>230 && col.g>230 && col.b>230;
}
...
async function loadIsoGeomsFiltered(map){
  ...
  for (const [cid, path] of Object.entries(map)){
    try{
      ...
      // nagyon világos / túl nagy háttér szűrése
      const filtered = items.filter(it=>{
        const b = boundsOfShape(it.shape);
        const huge = Uw>0 && Uh>0 && ((b.w>=0.80*Uw && b.h>=0.80*Uh) || b.w>=0.92*Uw || b.h>=0.92*Uh);
        return !huge && !isVeryLight(it.fill);
      });

      const parts = filtered.length ? filtered : items;
...
function animate(){
  ...
  if (detailLock) {
    ...
    // Hover esetén pixel-alapú küszöb
    const n = iconByKey[activeKey];
    if (n && shouldShowIso(n)) {
  ...
  if (hoverEnabled && hits.length){
    const fill = hits[0].object;
    const newKey = fill.userData.key;
